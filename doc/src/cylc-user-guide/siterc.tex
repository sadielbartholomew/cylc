...
\item {\em options:}
    \begin{myitemize}
    \item {\bf random} - rank for suitability and select a host at random
    \item {\bf load:1} - rank for suitability and select the host with the
lowest load average over 1 minute, in KB, (as given by the
\lstinline=uptime= command)
    ...
    \item {\bf memory} - rank for suitability and select the host with the
highest usable memory, in KB, as specified under \lstinline=/proc/meminfo=
    \item {\bf disk-space:PATH} - rank for suitability and select the host
with the highest free disk space for a given mount directory path
\lstinline=PATH=, in KB, as given by the \lstinline=df= command, where
multiple paths may be specified individually i.e. via
...
A list of metric measure and associated numeric value specifications
against which all configured run hosts are compared; if for a given allowed
host the extracted value of {\em any} of the provided measures does not
meet the cutoff provided by the corresponding numeric value, which is a
minimum or a maximum requirement depending on the measure context, the host
is excluded from the list of run hosts to undergo ranking i.e. excluded as a
potential run host. Usable memory (\lstinline=memory=) and free disk space
...
these corespond to all the rank methods accepted under the rank setting
...
\item {\em options:}
    \begin{myitemize}
    \item {\bf load:1} - load average over 1 minute, in KB, (as given by
the \lstinline=uptime= command)
    \item {\bf load:5} - as for \lstinline=load:1= above, but over 5 minutes
    \item {\bf load:15} - as for \lstinline=load:1= above, but over 15 minutes
    \item {\bf memory} - usable memory, in KB, as specified under
